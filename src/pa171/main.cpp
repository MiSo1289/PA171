...
//#include <pa171/image_compressor_lossless.hpp>
#include <pa171/image_compressor_lossy.hpp>
...
main(int const argc, char const* const* const argv) -> int
{
    ...
    auto const image_data =
        pa171::read_grayscale_image("data/forest.jpg", w, h);

    auto coder = pa171::image_compressor_lossy{};
    coder.set_region_size(32u);
    coder.set_transform_haar_wavelet();
...
//    decoder.set_transform_haar_wavelet();
    ...
    pa171::write_grayscale_image_as_bmp(
        "data/garfield_after_lossy.bmp", w, h, image_data.get());

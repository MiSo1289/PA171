...
class inverse_lazy_wavelet_transform
{
public:
  constexpr inverse_lazy_wavelet_transform(std::optional<T> const approx_norm,
...
template<typename T, std::size_t order, lifting_step<T, order>... Steps>
class lazy_wavelet_transform
{
public:
  constexpr lazy_wavelet_transform() = default;

  constexpr explicit lazy_wavelet_transform(Steps const&... steps) requires(
    sizeof...(Steps) > 0u)
    : steps_{ steps... }
  {
  }
  ...
  [[nodiscard]] constexpr auto predict_step(P const& predict) const
    -> lazy_wavelet_transform<T, order, Steps..., P>
  requires(sizeof...(Steps) % 2 == 0)
  {
    return std::apply(
      [&](Steps const&... steps)
      {
        return lazy_wavelet_transform<T, order, Steps..., P>{
          steps...,
          predict,
        };
      },
  ...
  [[nodiscard]] constexpr auto update_step(U const& update) const
    -> lazy_wavelet_transform<T, order, Steps..., U>
  requires(sizeof...(Steps) % 2 == 1)
  {
    return std::apply(
      [&](Steps const&... steps)
      {
        return lazy_wavelet_transform<T, order, Steps..., U>{
          steps...,
          update,
        };
      },
  ...
  [[nodiscard]] constexpr auto inverse() const
    -> inverse_lazy_wavelet_transform<T, order, Steps...>
  {
    return std::apply(
      [&](Steps const&... steps)
      {
        return inverse_lazy_wavelet_transform<T, order, Steps...>{
          approx_norm_,
          detail_norm_,
          steps...,
        };
...
constexpr auto haar_iwt = lazy_wavelet_transform<I, 1u>{}
...
constexpr auto db4_iwt = []
{
  ...
  return lazy_wavelet_transform<I, 2u>{}
...
constexpr auto haar_wt = []
{
  ...
  return lazy_wavelet_transform<F, 1u>{}
...
constexpr auto db4_wt = []
{
  ...
  return lazy_wavelet_transform<F, 2u>{}
...
template<typename T>
class full_2d_wavelet_transform
{
public:
  ...
  auto operator()(view_2d<I> input,
                  O result,
                  lazy_wavelet_transform<T, order, Steps...> const& wavelet)
    -> O
  {
    ...
    while (width > 1u or height > 1u)
    {
      ...
      for (auto const detail_part : { detail_vert, detail_diag, detail_hor })
      {
        result =
    ...
    //    result = std::ranges::copy(image_, result).out;
...
template<typename T>
class inv_full_2d_wavelet_transform
{
public:
  template<std::random_access_iterator I,
           std::output_iterator<T> O,
...
template<typename T>
class full_2d_inverse_wavelet_transform
{
public:
  template<std::random_access_iterator IO,
           std::size_t order,
           lifting_step<T, order>... Steps>
  requires std::output_iterator<IO, T>
  void operator()(
    IO const data,
    std::size_t const width,
    std::size_t const height,
    inverse_lazy_wavelet_transform<T, order, Steps...> const& inverse_wavelet)
  {
    buffer_.resize(std::max(width, height));

    backtrack(data, width, height, width, inverse_wavelet);
  }
...
private:
  ...
  void backtrack(
    ...
    inverse_lazy_wavelet_transform<T, order, Steps...> const& inverse_wavelet)
  {
    if (width <= 1u and height <= 1)
    {
      return;
    }

    auto const row = [&](auto const i)
    { return ranges::views::counted(data + i * row_stride, width); };

    auto const col = [&](auto const j)
    {
      return ranges::views::unbounded(data + j) |
             ranges::views::stride(row_stride) | ranges::views::take(height);
    };

    auto const prev_width = ceil_div(width, std::size_t{ 2 });
    auto const prev_height = ceil_div(height, std::size_t{ 2 });

    backtrack(data, prev_width, prev_height, row_stride, inverse_wavelet);

    if (width > 1u)
    {
      for (auto const i : std::views::iota(std::size_t{ 0 }, height))
      {
        inverse_wavelet(row(i) | ranges::views::take(prev_width),
                        row(i) | ranges::views::drop(prev_width),
                        buffer_.begin());
        std::ranges::copy_n(buffer_.begin(), width, row(i).begin());
      }
    }
    ...
    if (height > 1u)
    {
      for (auto const j : std::views::iota(std::size_t{ 0 }, width))
      {
        inverse_wavelet(col(j) | ranges::views::take(prev_height),
                        col(j) | ranges::views::drop(prev_height),
                        buffer_.begin());
        std::ranges::copy_n(buffer_.begin(), height, col(j).begin());
      }
    }
  }
};
